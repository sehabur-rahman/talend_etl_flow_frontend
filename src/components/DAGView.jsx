...
function DAGView({
  ...
  const updateEdgesForSelectedNode = useCallback(
    (nodeId, nodeType) => {
      if (!nodeId) {
  ...
  const filteredNodes = useMemo(() => {
    ...
    const matchingNodeDetails = allNodes.filter(
      (node) =>
        node.name.toLowerCase().includes(query) ||
        node.type.toLowerCase().includes(query) ||
        (node.desc && node.desc.toLowerCase().includes(query))
    ...
    console.log(matchingNodeDetails);

    getFilteredNodes(matchingNodeDetails);
  ...
  useEffect(() => {
    if (selectedNodes) {
      ...
      const matchingNode = nodes.find(
        (node) =>
          node.data.label.toLowerCase() == selectedNodes?.name?.toLowerCase() &&
          node.data.type.toLowerCase() == selectedNodes?.type?.toLowerCase() &&
          node.data.workspace.toLowerCase() ==
            selectedNodes?.workspace?.toLowerCase()
      );
      ...
      {selectedNode && (
        <div className="node-details">
          ...
          <div className="detail-item">
            <strong>Type:</strong>{" "}
            <span
              className={`type-badge type-${selectedNode.type}`}
              style={{ fontSize: "14px" }}
            >
              {selectedNode.type}
            </span>
          ...
          {selectedNode.schedule && (
            <>
              <div className="detail-item">
                ...
                {selectedNode.schedule.trigger_type}
              </div>
              <div className="detail-item">
                <strong>Trigger details:</strong>{" "}
                {selectedNode.schedule.trigger_details}
              </div>
            </>
          )}
          ...
          {selectedNode.artifact && (
            <>
              <div className="detail-item">
                <strong>Talend studio project:</strong>{" "}
                {selectedNode.artifact.talend_studio_project}
              </div>
              <div className="detail-item">
                <strong>Github repo:</strong>{" "}
                {selectedNode.artifact.git_repo_name}
              </div>
              <div className="detail-item">
                <strong>Repo last update:</strong>{" "}
                {selectedNode.artifact.repo_last_update}
              </div>
            </>
          )}
        </div>
